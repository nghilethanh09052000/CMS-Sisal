...
import { Typography, Button, TextField, FormControlLabel, Checkbox, Tooltip, Icon, Tabs, Tab } from '@material-ui/core'
...
class Overview extends React.Component
{
	constructor(props)
	{
		...
		this.actionsExtend = {
			createElement: (parentProps) =>
			{
				...
				const apiAdd = API.VersionAdd.bind(API)
				const apiUpdate = API.VersionEdit.bind(API)
				const apiDelete = API.VersionDelete.bind(API)
									...
									<CmsControlPermission
										control={
											<Button
												variant={'contained'}
												color={'primary'}
												onClick={this.handleAction('version_add', {version: '', client: '', env: '', goLive: false})}
												className={clsx(classes.buttonLeft)}
												startIcon={<Icons.IconAdd/>}
											>
												{ TEXT.OVERVIEW_BUTTON_NEW_VERSION }
											</Button>
										}
										link={''}
										attribute={''}
    ...
    componentDidMount()
	{
		this.props.SetTitle(TEXT.OVERVIEW_OVERVIEW_TITLE)

		this.props.VersionsLoad()
	...
	formatExcelData = () =>
	{
		let result = this.props.versions
	...
	handleAction = (name, data) => (evt) =>
	{
		switch (name)
		{
			...
			case 'submit':
				...
				{
					this.state.dialogType === 'version_add' && this.props.VersionAdd(this.state.rowData)
					this.state.dialogType === 'version_edit' && this.props.VersionEdit(this.state.rowData)
	...
	renderDialog = () =>
	{
		return (
			<ModalDialog
				...
				titleText={
                    this.state.dialogType === 'version_add' && TEXT.OVERVIEW_BUTTON_NEW_VERSION ||
                    this.state.dialogType === 'version_edit' && TEXT.OVERVIEW_BUTTON_EDIT_VERSION ||
			...
			{
				(this.state.dialogType === 'version_add' || this.state.dialogType === 'version_edit') && this.renderAddEditVersion()
			}
			</ModalDialog>
		)
	}
                ...
                <CmsTable
					...
					actions={[
						...
						},
						{
							...
							onClick: (event, rowData) =>
							{
								this.props.history.push(`/remote-configuration/detail/${rowData.id}/${encodeURIComponent(rowData.client)}/${rowData.version}/${rowData.env}/${rowData.status}/${rowData.goLive}`)
							},
							tooltip: (rowData) => ((this.state.isMultiSelectMode || rowData.deletedAt > 0) ? '' : TEXT.OVERVIEW_TOOLTIP_DETAIL),
							disabled: (rowData) => (this.state.isMultiSelectMode || rowData.deletedAt > 0),
							iconProps: { color: 'inherit' },
							position: 'row',
							controlPermission: {
						...
						},
						{
							...
							onClick: (event, rowData) =>
							{
								this.handleAction('version_edit', rowData)(event)
							},
							tooltip: (rowData) => ((this.state.isMultiSelectMode || rowData.deletedAt > 0) ? '' : TEXT.OVERVIEW_TOOLTIP_EDIT_VERSION),
							disabled: (rowData) => (this.state.isMultiSelectMode || rowData.deletedAt > 0),
							iconProps: { color: 'inherit' },
							position: 'row',
							controlPermission: {
...
const mapDispatchToProps = (dispatch) => ({
	...
	ClientsLoad: () =>
	{
		dispatch(ActionCMS.ClientsLoad())
	},
	DetailsLoad: () =>
	{
		dispatch(ActionCMS.DetailsLoad())
	},
	VersionsLoad: () =>
	{
		dispatch(ActionCMS.VersionsLoad())
	},
	VersionAdd: (version_data) =>
	{
		dispatch(ActionCMS.VersionAdd(version_data))
	},
