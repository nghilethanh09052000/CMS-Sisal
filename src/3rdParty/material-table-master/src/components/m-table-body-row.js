...
export default class MTableBodyRow extends React.Component {
  renderColumns() {
    ...
    const mapArr = this.props.columns.filter(columnDef => !columnDef.hidden && !(columnDef.tableData.groupOrder > -1))
      ...
      .map((columnDef, index) => {
        // LCD modified
        if (columnDef.tableData.width === 0) {
          // console.log('dont draw cell', columnDef.title)
          return null
        }
        ...
        if (
          ...
          return (
            <this.props.components.EditCell
              ...
              columnDef={{
                ...columnDef,
                cellStyle: {...this.props.options.cellStyle, ...columnDef.cellStyle},
              }}
              size={size}
              key={
                "cell-" +
              ...
              // long.toquoc disable action edit cell in add/update/delete row mode
          ...
          return (
            <this.props.components.Cell
              ...
              icons={this.props.icons}
              // long.toquoc edit push localization to props
              localization={this.props.localization}
              columnDef={{
                ...columnDef,
                cellStyle: {...this.props.options.cellStyle, ...columnDef.cellStyle},
  ...
  renderActions() {
    ...
    // LCD modified
    const actionsCellStyle = this.getActionStyle(this.props.index, this.props.level)
    return (
      <TableCell size={size} padding="none" key="key-actions-column" style={{ width: width, padding: '0px 5px', boxSizing: 'border-box', ...actionsCellStyle }}>
        { /* LCD  modified */}
  ...
  renderSelectionColumn() {
    ...
    return (
      /* TQLong  modified ...this.props.options.cellStyle */
      <TableCell size={size} padding="none" key="key-selection-column" style={{ width: selectionWidth, ...this.props.options.actionsCellStyle, ...this.props.options.cellStyle }}>
        { /* LCD  modified */}
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <Checkbox
            size={size}
  ...
  renderDetailPanelColumn() {
    ...
    if (typeof this.props.detailPanel == 'function') {
      return (
        /* TQLong  modified ...this.props.options.cellStyle */
        <TableCell size={size} padding="none" key="key-detail-panel-column" style={{ width: 42, textAlign: 'center', ...this.props.options.detailPanelColumnStyle, ...this.props.options.cellStyle }}>
          <IconButton
            size={size}
      ...
      return (
        <TableCell size={size} padding="none" key="key-detail-panel-column">
          /* TQLong  modified ...this.props.options.cellStyle */
          <div style={{ width: 42 * this.props.detailPanel.length, textAlign: 'center', display: 'flex', ...this.props.options.detailPanelColumnStyle, ...this.props.options.cellStyle }}>
  ...
  // LCD added
  getActionStyle(index, level) {
    let style = {};

    if (typeof this.props.options.actionsCellStyle === "function") {
      style = {
        ...style,
        ...this.props.options.actionsCellStyle(this.props.data, index, level)
      };
    }
    else if (this.props.options.actionsCellStyle) {
      style = {
        ...style,
