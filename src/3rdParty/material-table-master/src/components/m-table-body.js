  ...
  renderUngroupedRows(renderData) {
    return renderData.map((data, index) => {
      if (data.tableData.editing) {
        // long.toquoc render edit/delete row
        return (
  ...
  render() {
    ...
    if (this.props.options.paging) {
      ...
      if (this.props.pageSize > 0) {
        ...
        if(renderData.length <= this.props.pageSizeOptions[0]){
          emptyRowCount = this.props.pageSizeOptions[0] - renderData.length;
        } else if(renderData.length < this.props.pageSize){
          emptyRowCount = 0;
        }
      }
    }
    ...
    return (
      <TableBody>
        ...
        {/* long.toquoc render add row first */}
        ...
        {/* long.toquoc render add row last */}
