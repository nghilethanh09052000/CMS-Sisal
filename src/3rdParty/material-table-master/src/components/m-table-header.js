...
export class MTableHeader extends React.Component {
    ...
    const mapArr = this.props.columns.filter(columnDef => !columnDef.hidden && !(columnDef.tableData.groupOrder > -1))
      ...
      .map((columnDef, index) => {
        ...
        if (columnDef.tableData.width === 0) {
          // console.log('dont draw header', columnDef.title)
          return null
        }
        ...
        } else if (typeof(columnDef.title) === 'function') {
          content = columnDef.title()
        } else {
        ...
        if (this.props.draggable) {
          // LCD modified: fixed column title function not working
          let title = content
          content = (
            <Draggable
  ...
  renderSelectionHeader() {
    ...
    if (typeof checkboxProps === 'function') {
      checkboxProps = checkboxProps({ checkedAll: this.props.dataCount > 0 && this.props.selectedCount === this.props.dataCount });
    }
    ...
    return (
      <TableCell
        ...
        className={this.props.classes.header}
        style={{ ...this.props.headerStyle, ...this.props.options.actionsCellStyle, width: selectionWidth }}
      >
  ...
  renderDetailPanelColumnCell() {
    return <TableCell
      ...
      className={this.props.classes.header}
      // long.toquoc add "width: CommonValues.baseIconSize(this.props)" to fix CSS
      style={{ ...this.props.headerStyle, width: CommonValues.baseIconSize(this.props) }}
    />;
  }
