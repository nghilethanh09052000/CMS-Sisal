  ...
  renderBooleanFilter = (columnDef) => (
    ...
    <FormControlLabel
      control={
        <Checkbox
          color={'primary'}
          checked={columnDef.tableData.filterValue || false}
          onChange={(evt, checked) => {
            this.props.onFilterChanged(columnDef.tableData.id, checked);
          }}
        />
      }
      label={columnDef.filterPlaceholder}
      labelPlacement={'end'}
    />
  )
  ...
  renderDefaultFilter = (columnDef) => {
    ...
    return (
      <TextField
        ...
        onChange={(event) => {
          this.props.onFilterChanged(columnDef.tableData.id, event.target.value, false);
        }}
        // long.toquoc add
        onKeyPress={(event) => {
          if (event.key === 'Enter') {
            event.preventDefault();
            this.props.onFilterChanged(columnDef.tableData.id, event.target.value, true);
          }
        }}
