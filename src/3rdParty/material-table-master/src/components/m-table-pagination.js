...
class MTablePaginationInner extends React.Component {
  ...
  render() {
    const { classes, count, page, theme, showFirstLastPageButtons } = this.props;
    const localization = { ...MTablePaginationInner.defaultProps.localization, ...this.props.localization };
    // LCD modified
    const rowsPerPage = this.props.rowsPerPage < 0 ?  this.props.count : this.props.rowsPerPage
    return (
      <div className={classes.root}>
        ...
        <Typography variant="caption" style={{ flex: 1, textAlign: 'center', alignSelf: 'center', flexBasis: 'inherit' }} >
          {/* LCD modified */}
          {localization.labelDisplayedRows.replace('{from}', this.props.count === 0 ? 0 : ((this.props.page * rowsPerPage) + 1)).replace('{to}', Math.min((this.props.page + 1) * rowsPerPage, this.props.count)).replace('{count}', this.props.count)}
        </Typography>
