...
export default class DataManager {
  ...
  setColumns(columns) {
    ...
    this.columns = columns.map((columnDef, index) => {
      ...
      // LCD modified
      let searchAllColumns = this.searchColumnFilter === 'All'
      if (columnDef.field === this.searchColumnFilter || columnDef.title === this.searchColumnFilter) {
        columnDef.searchable = true
      } else {
        columnDef.searchable = searchAllColumns
      }
  ...
  // LCD modified
  changeSearchAlgorithm(algorithm) {
    this.searchAlgorithm = algorithm

    this.columns.forEach((column, index) => {
      if (column.tableData.filterValue) {
        this.changeFilterValue(index, column.tableData.filterValue)
      }
    })
  }

  changeSearchColumnFilter(columnFilter) {
    this.searchColumnFilter = columnFilter
    let searchAllColumns = columnFilter === 'All'

    this.columns.forEach(column => {
      if (column.field === columnFilter || column.title === columnFilter) {
        column.searchable = true
      } else {
        column.searchable = searchAllColumns
      }
    })
  }
    ...
    if (this.applyFilters) {
      this.columns.filter(columnDef => columnDef.tableData.filterValue).forEach(columnDef => {
        ...
        if (columnDef.customFilterAndSearch) {
          this.filteredData = this.filteredData.filter(row => !!columnDef.customFilterAndSearch(tableData.filterValue, row, { ...columnDef, searchAlgorithm: this.searchAlgorithm }));
        }
        else {
          if (lookup) {
    ...
    if (this.searchText && this.applySearch) {
      this.searchedData = this.searchedData.filter(row => {
        return this.columns
          ...
          .some(columnDef => {
            if (columnDef.customFilterAndSearch) {
              return !!columnDef.customFilterAndSearch(this.searchText, row, { ...columnDef, searchAlgorithm: this.searchAlgorithm });
            }
            else if (columnDef.field) {
              const value = this.getFieldValue(row, columnDef);
              if (value) {
                // LCD modified
                if (this.searchAlgorithm === 'includes') {
                  return value.toString().includes(this.searchText);
                }
                return value.toString().startsWith(this.searchText);
              }
            }
          });
      });
